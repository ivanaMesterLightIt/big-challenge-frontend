...
export const Select: FC<SelectProps> = ({ options, defaultSelected, onselect }) => {
            ...
            <Transition show={open} as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100"
                ...
                <Listbox.Options
                    ...
                    <Listbox.Option key={option.id} className={({ active })=>
                        ...
                        {({ selected, active }) => (
                        <>
                            <span className={tw(selected ? 'font-semibold' : 'font-normal' , 'block truncate' )}>
                                {option.name}
                            </span>

                            {selected ? (
                            <span className={tw( active ? 'text-white' : 'text-blue-600'
